...
```
Next.js App → GraphQL Gateway → gRPC/REST Services
```

**Option 2: Direct REST API**
```
Next.js App → REST Services → Database
```

**Option 3: BFF Pattern**
```
Next.js App → Dedicated BFF (GraphQL/REST) → Multiple Services
```

**SSR Considerations**:
- Server-side rendering can directly call backend services
- API routes can act as a lightweight BFF layer
